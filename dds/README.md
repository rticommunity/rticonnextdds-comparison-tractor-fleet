...
| File | Purpose | Status |
|------|---------|--------|
| `robot_types.xml` | DDS type definitions (IDL-XML) — **single source of truth** for all topic types, enums, and structs | ✅ Complete (3 enums, 24 structs incl. CoveragePoint) |
...
| `run_demo.sh` | Uniform launch script: `all`, `robots`, `stations`, `ui`, `robot <name>`, `station <name>` | ✅ Complete |
| `stop_demo.sh` | Kill all running robot, station, and UI processes | ✅ Complete |
| `run_persistence_service.sh` | Launch RTI Persistence Service for durable CoveragePoint data | ✅ Complete |
...
### Current types (in `robot_types.xml`)

The types are fully implemented in `robot_types.xml`.  Key data types,
...
### Key Enums (defined in `robot_types.xml`)
...
expressed as DDS IDL enumerations in the XML type definitions.

### Code Generation with `rtiddsgen`

`robot_types.xml` is the **single source of truth** for all data types.  Python
dataclasses are generated — never hand-written:

```bash
$NDDSHOME/bin/rtiddsgen -language python robot_types.xml
```

If you want a plain `.idl` rendering of the XML definitions for inspection,
review, or sharing with non-XML tooling:

```bash
./convert_xml_to_idl.sh
```

This uses `rtiddsgen -convertToIdl -inputXml robot_types.xml` and writes
`robot_types.idl` next to the source XML file.  You can also pass a different
input XML path; the script always writes the matching `.idl` beside that input.
...
1. Edit `robot_types.xml` (add/remove/rename fields, types, or enums)
2. Re-run `rtiddsgen -language python robot_types.xml`
3. Optionally run `./convert_xml_to_idl.sh` if you also want a plain `.idl`
  snapshot for review or export
4. All Python code that imports from `robot_types` picks up the change — no
...
> from `robot_types.xml`.  Same workflow — different IDL, different middleware,
...
```bash
# (Re-)generate Python types from XML (only needed after editing robot_types.xml):
$NDDSHOME/bin/rtiddsgen -language python robot_types.xml
